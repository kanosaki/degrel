...
object ParseTestUtils {
  def mkVertex(lbl: String, edges: Iterable[(String, AstVertex)] = Map()) = {
    AstVertex(AstName(None, Some(AstLabel(lbl))), mkEdges(edges))
  }
...
class DefaultTermParserTest extends FlatSpec {
  ...
  it should "parse rules" in {
    assert_graph("foo(bar: baz) -> hoge(fuga: piyo)",
      AstRule(
        mkVertex("foo", Seq(("bar", mkVertex("baz")))),
  ...
  it should "parse rules with capturing" in {
    assert_graph("foo(bar: X[baz]) -> hoge(fuga: X)",
      AstRule(
        mkVertex("foo", Seq(("bar",
          AstVertex(mkName(label = "baz", capture = "X"), Seq())))),
        mkVertex("hoge", Seq(("fuga",
          AstVertex(mkName(capture = "X"), Seq()))))
      )
    )
  }
